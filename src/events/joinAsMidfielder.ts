...
import { BRONZE, SILVER, ELO_GAP } from "../utils/src/constants";
import { User } from "../utils/mongodb/Models";



const customId = 'join_as_midfielder_'

export const JoinAsMidfielder: buttonInteractionType = {
    customId,
    run: async(client: Client, interaction: Interaction) => {
        ...
        if (check.type == 'solo') {
            if (check.ranked?.bronze?.midfielders.includes(user) || check.ranked?.silver?.midfielders.includes(user) || check.ranked?.gold?.midfielders.includes(user)) {
                await interaction.reply({
            ...
            if (userData.soloRankedElo < BRONZE - ELO_GAP) {
                await Lineup.findOneAndUpdate({
                    ...
                    $addToSet: {'ranked.bronze.midfielders': user},
                    $pull: {
            ...
            if (userData.soloRankedElo >= BRONZE - ELO_GAP && userData.soloRankedElo < SILVER - ELO_GAP) {
                await Lineup.findOneAndUpdate({
                    ...
                    $addToSet: {'ranked.bronze.midfielders': user, 'ranked.silver.midfielders': user},
                    $pull: {
                        'ranked.bronze.attackers': user,
                        //'ranked.bronze.midfielders': user,
                        'ranked.bronze.defenders': user,
            ...
            if (userData.soloRankedElo > BRONZE - ELO_GAP && userData.soloRankedElo < SILVER - ELO_GAP) {
                await Lineup.findOneAndUpdate({
                    ...
                    $addToSet: {'ranked.silver.midfielders': user},
                    $pull: {
                        'ranked.bronze.attackers': user,
                        'ranked.bronze.midfielders': user,
                        'ranked.bronze.defenders': user,
            ...
            if (userData.soloRankedElo >= SILVER - ELO_GAP && userData.soloRankedElo < SILVER) {
                await Lineup.findOneAndUpdate({
                    ...
                    $addToSet: {'ranked.silver.midfielders': user, 'ranked.gold.midfielders': user},
                    $pull: {
                        'ranked.bronze.attackers': user,
                        'ranked.bronze.midfielders': user,
                        'ranked.bronze.defenders': user,
            ...
            if (userData.soloRankedElo > SILVER + ELO_GAP) {
                await Lineup.findOneAndUpdate({
                    ...
                    $addToSet: {'ranked.gold.midfielders': user},
                    $pull: {
                        'ranked.bronze.attackers': user,
                        'ranked.bronze.midfielders': user,
                        'ranked.bronze.defenders': user,
